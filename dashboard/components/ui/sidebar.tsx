...
const SIDEBAR_WIDTH = "calc((100vw - var(--sides) * 2 - var(--gap) * 11) * 2 / 12 + var(--gap))"
    ...
    <SidebarContext.Provider value={contextValue}>
      <TooltipProvider delayDuration={0}>
        <div
          ...
          className={cn("group/sidebar-wrapper has-data-[variant=inset]:bg-sidebar min-h-svh", className)}
          {...props}
        >
          {children}
        </div>
    ...
    >
      ...
      >
        <div
          ...
          className="group-data-[variant=floating]:border-sidebar-border flex h-full w-full flex-col gap-1 group-data-[variant=floating]:rounded-lg group-data-[variant=floating]:border group-data-[variant=floating]:shadow-elevated shadow-sidebar dark:shadow-orange-glow"
        >
          {children}
        </div>
      </div>
...
function SidebarHeader({ className, ...props }: React.ComponentProps<"div">) {
  return (
    <div
      ...
      data-sidebar="header"
      className={cn("flex flex-col gap-2 p-2 rounded-lg bg-sidebar ring-2 ring-sidebar-foreground/[0.025]", className)}
      {...props}
    />
  )
}
...
const sidebarMenuButtonVariants = cva(
  "peer/menu-button flex w-full items-center gap-3 overflow-hidden rounded p-2 text-left text-sm uppercase outline-hidden ring-sidebar-ring transition-[width,height,padding] focus-visible:ring-2 disabled:pointer-events-none disabled:opacity-50 group-has-data-[sidebar=menu-action]/menu-item:pr-8 aria-disabled:pointer-events-none aria-disabled:opacity-50 data-[active=true]:bg-primary data-[active=true]:text-primary-foreground data-[active=true]:[&>svg]:text-primary-foreground hover:data-[active=false]:bg-sidebar-accent/50 hover:data-[active=false]:text-sidebar-accent-foreground group-data-[collapsible=icon]:size-8! group-data-[collapsible=icon]:p-2! [&>span:last-child]:truncate [&>svg]:size-5 [&>svg]:shrink-0",
  {
    variants: {
      variant: {
